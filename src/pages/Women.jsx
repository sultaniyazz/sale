  ...
  return (
    <div className="min-h-screen flex flex-col justify-between">
      <div className="relative flex-1   mx-auto max-w-7xl px-4  sm:px-6 lg:px-8 py-10 w-full">
        {/* Fullscreen pink+white gradient background */}
        <div className="fixed inset-0 -z-10 bg-gradient-to-br from-pink-10 via-white to-pink-20" />
        <div className="mx-auto max-w-7xl px-4">
          <h2 className="text-3xl font-bold text-pink-700 mb-10 text-center tracking-tight">Ayollar uchun mahsulotlar</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2">
            {womenProducts.map((product) => (
              <Link
                to={`/detail/${product.slug}`}
                key={product.id + product.name}
                className="relative bg-white rounded-3xl z-10 shadow-xl hover:shadow-pink-100 transition-shadow duration-300 flex flex-col overflow-hidden border border-pink-100 group"
              >
                {/* Like button */}
                <button
                  type="button"
                ...
                >
                  <Heart
                    className={`size-6 transition-colors ${liked[product.id + product.name] ? 'fill-red-500 text-red-500' : 'text-gray-300'}`}
                    fill={liked[product.id + product.name] ? 'currentColor' : 'none'}
                  />
                </button>
                {/* Product image */}
                <div className="bg-gradient-to-t from-pink-10 via-white to-white p-5 pb-0 flex items-center justify-center">
                  <img
                    alt={product.name}
                    src={product.image}
                ...
                {/* Brand & rating bar */}
                <div className="flex items-center justify-between px-6 py-3 border-t bg-pink-10">
                  <span className="text-sm font-medium text-gray-600">{product.brand}</span>
                  <span className="text-sm flex items-center gap-1 text-pink-10 font-semibold">
